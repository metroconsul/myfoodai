...
export const Route = createFileRoute("/_authenticated/app/conformidade/exames/$id")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Detalhes, prazos, arquivo e trilha de auditoria do documento.",
      },
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <SectionCard title="Dados do documento">
          <dl className="grid gap-3 text-sm sm:grid-cols-2">
            <div>
              <dt className="meta-mono">Realização</dt>
              <dd>{dateFmt(doc.performed_at)}</dd>
            </div>
            <div>
              <dt className="meta-mono">Validade</dt>
              <dd>{dateFmt(doc.expires_at)}</dd>
            </div>
            <div>
              <dt className="meta-mono">Próxima revisão</dt>
              <dd>{dateFmt(doc.next_review_at)}</dd>
            </div>
            <div>
              <dt className="meta-mono">Responsável</dt>
              <dd>{doc.provider_name ?? "—"}</dd>
            </div>
            <div>
              <dt className="meta-mono">Referência</dt>
              <dd>{doc.provider_reference ?? "—"}</dd>
            </div>
            <div>
              <dt className="meta-mono">Próxima ação</dt>
              <dd>{doc.next_action ?? "—"}</dd>
            </div>
          </dl>
          {health ? (
            <p className="mt-4 flex items-start gap-2 rounded-[10px] border-2 border-foreground bg-warning p-3 text-xs">
              <ShieldAlert className="mt-0.5 size-4 shrink-0" aria-hidden />
              Documento de saúde com acesso restrito. O sistema não interpreta resultados nem
              determina aptidão.
            </p>
          ) : null}
        </SectionCard>
        ...
        <SectionCard title="Arquivo">
          {doc.file_path ? (
            canSeeFile ? (
              <div className="space-y-2">
                ...
                <p className="meta-mono">
                  {doc.file_size ? `${(doc.file_size / 1024).toFixed(0)} KB` : "—"} · enviado em{" "}
                  {dateFmt(doc.created_at)}
                </p>
                <Button
                  onClick={() => download.mutate(doc.file_path!)}
                  disabled={download.isPending}
                >
                  <Download className="mr-2 size-4" /> Baixar documento
                </Button>
                <p className="text-xs text-muted-foreground">
                  O download é registrado na trilha de auditoria.
                </p>
              </div>
            ) : (
              <EmptyState
            ...
            <EmptyState
              title="Sem arquivo"
              description="Nenhum arquivo foi anexado a este documento."
            />
          )}
        </SectionCard>
      </div>
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          {!doc.published_to_portal_at ? (
            <Button
              ...
              disabled={update.isPending}
              onClick={() =>
                update.mutate({ published_to_portal_at: new Date().toISOString(), is_draft: false })
              }
            >
              Publicar no portal
            </Button>
      ...
      <SectionCard title="Confirmação do colaborador">
        {query.data.acknowledgement ? (
          <dl className="grid gap-3 text-sm sm:grid-cols-2">
            <div>
              <dt className="meta-mono">Visualizado em</dt>
              <dd>{dateTimeFmt(query.data.acknowledgement.viewed_at)}</dd>
            </div>
            <div>
              <dt className="meta-mono">Ciência</dt>
              <dd>{dateTimeFmt(query.data.acknowledgement.acknowledged_at)}</dd>
            </div>
            <div>
              <dt className="meta-mono">Assinatura</dt>
              <dd>{dateTimeFmt(query.data.acknowledgement.signed_at)}</dd>
            </div>
            <div>
              <dt className="meta-mono">Localização</dt>
              <dd>{query.data.acknowledgement.location_status}</dd>
            </div>
            <div className="sm:col-span-2">
              <dt className="meta-mono">Hash de integridade</dt>
              <dd className="break-all text-xs">
                {query.data.acknowledgement.integrity_hash ?? "—"}
              </dd>
            </div>
          </dl>
        ) : (
          <EmptyState
            ...
            description="O colaborador ainda não confirmou este documento."
          />
        )}
      </SectionCard>
      ...
      <SectionCard title="Trilha de auditoria">
        {query.data.logs.length === 0 ? (
          <EmptyState
            title="Sem eventos"
            description="Visualizações, downloads e assinaturas aparecem aqui."
          />
        ) : (
            ...
            {query.data.logs.map((log) => (
              <li key={log.id} className="flex items-center justify-between gap-3 py-2 text-sm">
                ...
                <span className="meta-mono">
                  {log.actor_type} · {dateTimeFmt(log.created_at)}
                </span>
              </li>
            ))}
          </ul>
