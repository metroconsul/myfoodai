REVOKE ALL ON FUNCTION public.set_updated_at() FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.handle_new_user() FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.has_role(uuid, public.app_role) FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.current_company_id() FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.is_company_admin() FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.can_see_all_units() FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.has_unit_access(uuid) FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.in_company(uuid) FROM PUBLIC, anon;

GRANT EXECUTE ON FUNCTION public.has_role(uuid, public.app_role) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.current_company_id() TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.is_company_admin() TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.can_see_all_units() TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.has_unit_access(uuid) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.in_company(uuid) TO authenticated, service_role;